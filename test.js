...
window.onload = (event) => {
  ...
  const statusOfWidget = true;
  const nameOption = true;
  const emailOption = false;
  const surnameOption = false;
  const couponOption = true;
  ...
  ${emailOption ? `<div class="element-container">
  ...
  <span> Aggiungi Coupon</span></div>` : null}
